<div class="modal _project_file" tabindex="-1" role="dialog" data-toggle="modal">
	<div class="modal-dialog full-screen-modal w-100" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-md-12 border-right project_file_area">
						...
						<?php } ?>
						<?php
						$path = HR_PROFILE_FILE_ATTACHMENTS_UPLOAD_FOLDER.'/' .$file->rel_id.'/'.$file->file_name;
						if(is_image($path)){ ?>
							<img src="<?php echo base_url('modules/hr_profile/uploads/att_file/'.$file->rel_id.'/'.$file->file_name); ?>" class="img img-responsive" >
						...
						<?php } else if(strpos($file->filetype,'pdf') !== false && empty($file->external)){ ?>
							<iframe src="<?php echo base_url('modules/hr_profile/uploads/att_file/'.$file->rel_id.'/'.$file->file_name); ?>" height="100%" width="100%" frameborder="0"></iframe>
						<?php } else if(strpos($file->filetype,'xls') !== false && empty($file->external)){ ?>
							<iframe src='https://view.officeapps.live.com/op/embed.aspx?src=<?php echo base_url('modules/hr_profile/uploads/att_file/'.$file->rel_id.'/'.$file->file_name); ?>' width='100%' height='100%' frameborder='0'>
							</iframe>
						...
						<?php } else if(is_markdown_file($path) && $previewMarkdown = markdown_parse_preview($path)) {
							...
							echo '<a href="'.site_url('modules/hr_profile/uploads/att_file/'.$file->rel_id.'/'.$file->file_name).'" download>'.$file->file_name.'</a>';
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default" onclick="close_modal_manually('._project_file'); return false;"><?php echo _l('hr_close'); ?></button>
			</div>
		</div>
	</div>
</div>
<?php 
require('modules/hr_profile/assets/js/preview_file_js.php');
?>
