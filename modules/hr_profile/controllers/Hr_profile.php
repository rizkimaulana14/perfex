...
public function delete_staff()
{
  ...
  redirect(admin_url('hr_profile/staff_infor'));
	...
	{
		$this->app->get_table_data(module_views_path('hr_profile', 'hr_record/table_education'));
	}
...
public function update_staff_profile(){
  ...
  if ($data['id'] == '') {
	...
	public function delete_error_file_day_before()
	{
		...
		foreach(glob(HR_PROFILE_ERROR . '*') as $file) {
			...
			if($this->input->post()){
				...
				if($id_department == ''){
					...
					foreach ($list_diploma as $diploma) {
						...
						if(count($department_filter)==1){//one department
							$circle_mode = true;
