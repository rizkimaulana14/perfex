...
class App_vendor_area_constructor
{
    ...
    public function __construct()
    {
        ...
        $this->ci->load->model('purchase/authentication_vendor_model');

        $vars = [];
        if (is_client_logged_in()) {
            $contact            = $this->ci->purchase_model->get_contact(get_contact_user_id());
            ...
            $vars['client']                          = $this->ci->purchase_model->get_vendor($contact->userid);
